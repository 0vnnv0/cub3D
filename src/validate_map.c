...
/*   Updated: 2025/02/14 12:24:48 by anschmit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (y >= 0) // Start from the bottom
    {
        ...
        while ((*game)->map[y][x] != '\0') // Left to right
        {
                ...
                (*game)->map[y][x] == 'E' || (*game)->map[y][x] == 'W')
            {
                ...
                (*game)->player.x = x;
                (*game)->player.y = (*game)->height - 1 - y; // Invert y-axis
                return (1);
            }
            x++;
        }
...
static int validate_map_chars(t_game *game)
{
    ...
    if (player_count != 1)
    {
        printf("Error: Map must contain exactly one player, found %d\n", player_count);
        return (0);
    }
    return (1);
}
...
static void flood_fill(t_game *game, int **visited)
{
    ...
    while (top >= 0)
    {
        ...
        {
            printf("Skipping invalid access at (%d, %d)\n", x, y);
            continue;
        }
...
static int check_accessibility(t_game *game, int **visited)
{
    ...
    while (y < game->height)
    {
        ...
        while (x < row_len)
        {
            ...
            {
                printf("Error: Inaccessible area found at (%d, %d)\n", x, game->height - y - 1);
                return (0);
            }
            x++;
        }
    ...
    while (y < game->height)
    {
        ...
        while (x < row_len)
        {
            ...
            if ((tile == '0' || tile == 'N' || tile == 'S' || tile == 'E' || tile == 'W') &&
				...
				map[y][x + 1] == '\n'))
            {
                printf("Error9: Map not enclosed at (%d, %d)\n", x, game->height - y - 1);
                return (0);
            }
            x++;
        }
