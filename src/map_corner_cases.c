...
//fucntions to compare location of each two first char of each two lines and coloumns (case one, two and three)
//to compare location of each two last char of each two lines and coloumns (case four and five)
int	case_one(t_game *game, char **map, int y)
{
	...
	if(first_y > first_y_next)
	{
		...
		{
			print_error("Error1: Map not enclosed at (%d, %d)\n", first_y, game->height - y - 1);
			return (0);
		}
	}	
	else if(first_y < first_y_next)
	{
		...
		{
			print_error("Error2: Map not enclosed at (%d, %d)\n", first_y, game->height - y - 1);
			return (0);
		}
	}
	return (1);
...
int	case_two(t_game *game, char **map, int y)
{
	...
	if((last_y > last_y_next))
	{
		...
		{
			printf("Error3: Map not enclosed at (%d, %d)\n", last_y_next, game->height - y - 1);
			return (0);
		}
	}
	else if(last_y < last_y_next) 
	{
		if((map[y][last_y]) != '1' || (map[y+1][last_y]) != '1')
		{
			printf("y (%d)\n", y);
			printf("last y (%d)\n", last_y);
			printf("last y next (%d)\n", last_y_next);
			printf("Error4: Map not enclosed at (%d, %d)\n", last_y, game->height - y - 1);
			return (0);
		}
	}
	return (1);
...
int	case_three(t_game *game, char **map, int x)
{
	...
	{
		if (map[first_x_next] && (map[first_x_next][x] != '1' || map[first_x_next][x + 1] != '1'))
		{
			printf("Error5: Map not enclosed at (%d, %d)\n", x + 1, game->height - first_x_next);
			return (0);
		}
	}
	else if (first_x > first_x_next)
	{
		...
		{
			printf("Error6: Map not enclosed at (%d, %d)\n", x, game->height - first_x);
			return (0);
		}
	}
	return (1);
	...
	if (last_x > last_x_next)
	{
		if (last_x_next >= 0 && last_x_next < game->height && map[last_x_next])
		{
			if (x >= 0 && x < strlen(map[last_x_next]) - 1)
			{
				...
				{
					printf("Error7: Map not enclosed at (%d, %d)\n", x, game->height - last_x_next);
					return (0);
				}
			}
			else
	...
	if (last_x < last_x_next)
	{
		if (last_x >= 0 && last_x < game->height && map[last_x])
		{
			if (x >= 0 && x < strlen(map[last_x]) - 1)
			{
				...
				{
					printf("Error8: Map not enclosed at (%d, %d)\n", x + 1 , game->height - last_x - 1);
					return (0);
				}
			}
			else
